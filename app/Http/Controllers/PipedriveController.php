...
class PipedriveController extends Controller
{
    ...
    {
        $retorno = \App\Appointment::where('user_id',$id)->where('status','=', 0)->first();
        $retorno->status = 1;
        $retorno->fechado = 1;
       ...
       // $params = array('user_id',$usuario->pipedrive_id);
        try {
            $pipe->change_deal($retorno->deal_id , $usuario->pipedrive_id);
        } catch (Exception $e) {
            echo $e->getMessage();
        }
        

       // https://roleta.capriatti.com.br/altera_lead/$retorno->deal_id/$usuario->pipedrive_id
